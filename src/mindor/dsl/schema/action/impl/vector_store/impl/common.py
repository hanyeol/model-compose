...
class VectorStoreFilterCondition(BaseModel):
    field: str = Field(..., description="Field name to apply the filter on.")
    operator: VectorStoreFilterOperator = Field(default=..., description="Comparison operator for filtering.")
...
class CommonVectorStoreActionConfig(CommonActionConfig):
    ...
    id_field: str = Field(default="id", description="Field name identifying vectors in the store.")
    vector_field: str = Field(default="vector", description="Field name storing vector embeddings.")
    batch_size: Union[int, str] = Field(default=0, description="Number of items per batch.")
