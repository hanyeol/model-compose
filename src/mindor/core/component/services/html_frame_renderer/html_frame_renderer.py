...
from .base import HtmlFrameRendererDriver, HtmlFrameRendererDriverRegistry
import importlib
...
class HtmlFrameRendererComponent(ComponentService):
    def __init__(
        ...
        self.driver: HtmlFrameRendererDriver = self._create_driver(self.config.driver)

    def _create_driver(self, driver: HtmlFrameRendererDriverType) -> HtmlFrameRendererDriver:
        try:
            if driver not in HtmlFrameRendererDriverRegistry:
                self._load_driver_module(driver)
            return HtmlFrameRendererDriverRegistry[driver](self.id, self.config, self.daemon)
        except KeyError:
            raise ValueError(f"Unsupported html_frame_renderer driver: {driver}")

    def _load_driver_module(self, driver: HtmlFrameRendererDriverType) -> None:
        """Import the module that registers the given html_frame_renderer driver.

        Convention: a driver "foo-bar" (HtmlFrameRendererDriverType.value) maps to
        ...
        Importing the module triggers its @register_html_frame_renderer_driver
        decorator, populating HtmlFrameRendererDriverRegistry.
        """
