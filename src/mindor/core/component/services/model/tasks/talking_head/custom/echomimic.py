...
class EchoMimicTalkingHeadTaskDriver(ModelTaskDriver):
    ...
    def _build_pipeline(self, pipeline_class: type, config: Any, inference_config: Any, module_name: str, device: torch.device) -> Any:
        ...
        conditioner.to(dtype=weight_dtype, device=device)

        audio_processor = audio_loader(model_path=config.audio_model_path, device=device)
