...
class TestHttpClientActionConfig:
    ...
    def test_full_config_with_endpoint(self):
        ...
        config = HttpClientActionConfig(
            ...
            params={"version": "1.0"},
            stream_format=HttpEventStreamFormat.JSON
        )
        assert config.endpoint == "https://api.example.com/v1/chat"
        assert config.method == "POST"
        ...
        assert config.stream_format == HttpEventStreamFormat.JSON
    ...
    def test_streaming_formats(self):
        ...
        assert json_config.stream_format == HttpEventStreamFormat.JSON

        # TEXT format
        text_config = HttpClientActionConfig(
            endpoint="https://api.example.com/stream",
            stream_format=HttpEventStreamFormat.TEXT
        )
        assert text_config.stream_format == HttpEventStreamFormat.TEXT
...
class TestHttpClientIntegration:
    ...
    def test_streaming_api_pattern(self):
        ...
        assert config.actions[0].stream_format == HttpEventStreamFormat.JSON
