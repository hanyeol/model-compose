...
4. **자동 모델 관리**: 베이스 체크포인트와 모션 어댑터는 첫 실행 시 HuggingFace Hub에서 다운로드되어 로컬에 캐시됩니다.
...
2. 추가 환경 구성은 필요하지 않습니다 — 베이스 체크포인트(`Realistic_Vision_V5.1_noVAE`)와 모션 어댑터(`animatediff-motion-adapter-v1-5-3`)가 첫 실행 시 HuggingFace Hub에서 다운로드되어 `~/.cache/huggingface/` 아래에 캐시됩니다.
...
| `params.num_frames`           | 입력 영상에서 샘플링해 출력할 프레임 수. ~32프레임을 넘어가면 품질이 저하됩니다.                                        | `16`                                                     |
| `params.fps`                  | 출력 영상 프레임 레이트.                                                                                                | `8`                                                      |
...
- **프레임 수가 중요합니다**: AnimateDiff는 ~16프레임 창으로 학습되었습니다. ~32프레임을 넘어가면 눈에 띄는 드리프트와 모션 불일치가 발생하기 쉽습니다.
