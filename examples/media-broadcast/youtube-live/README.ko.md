...
이 예제는 공유된 `data-queue`로 구동되는 연속 YouTube Live 방송을 보여줍니다. 한 워크플로우는 필요할 때마다 비디오 소스를 큐에 넣고, 별도의 장기 실행 워크플로우가 큐를 소비하여 각 비디오를 RTMP로 YouTube Live에 스트리밍합니다.

## 개요

두 워크플로우가 하나의 in-process `data-queue` 컴포넌트 인스턴스를 공유합니다:

1. **publish-video**: 호출당 하나의 비디오 소스(파일 경로 또는 URL)를 큐에 넣습니다. 원하는 만큼 반복 호출해서 여러 비디오를 줄 세울 수 있습니다.
2. **broadcast-live**: 계속 실행됩니다 — 큐를 구독하여 스트림을 `rtmp-publisher` 컴포넌트에 곧바로 전달하고 YouTube Live로 송출합니다. 비디오는 FIFO 순서로 방송되며, 워크플로우는 취소될 때에만 정지합니다.

컴포넌트 인스턴스는 워크플로우 호출을 넘나들며 id로 캐시되므로, 두 워크플로우는 동일한 `asyncio.Queue`를 공유합니다. 컨슈머의 `dequeue` 액션은 `AsyncIterator`를 반환하며 `rtmp-publisher`가 이를 투명하게 순회하므로 별도의 `for-each` 연결이 필요하지 않습니다. 각 publish는 현재 비디오의 스트리밍이 끝날 때까지 블로킹되므로 방송이 끊김 없이 이어지고, 다음 비디오가 이전 비디오가 끝나는 즉시 시작됩니다.
...
2. **방송 시작 (계속 실행 상태로 두기):**
   ...
   터미널이나 탭 하나에서 컨슈머 워크플로우를 시작합니다. 첫 비디오가 도착할 때까지 대기합니다:
   ...
   첫 비디오가 큐에 들어가면 YouTube Studio 실시간 대시보드에 RTMP 피드가 들어오기 시작합니다. 공개 방송을 시작할 준비가 되면 YouTube Studio에서 **실시간 스트리밍 시작**을 클릭하세요.

3. **비디오 넣기 (반복 가능):**

   다른 터미널(또는 Web UI)에서 방송할 비디오마다 한 번씩 `publish-video`를 호출합니다:
...
- **드라이버**: `ffmpeg`
- **목적**: 큐에서 꺼낸 각 비디오를 인코딩하여 RTMP로 YouTube Live에 송출
- **주요 옵션**:
...
**설명**: 비디오 소스 하나를 `video-queue`에 push합니다. 반복 호출로 방송 재생목록을 쌓아 올립니다.
...
| 매개변수 | 타입 | 필수 | 기본값 | 설명 |
|-----------|------|----------|---------|-------------|
| `video` | file | 예 | - | 비디오 소스: 로컬 파일 경로, `file://` URL, 또는 `http(s)://` URL |
...
**설명**: 비디오 참조를 계속 dequeue하여 각각을 RTMP로 YouTube Live에 송출합니다. 취소될 때까지 실행됩니다.

#### 작업 흐름

1. **subscribe**: `video-queue`에서 consume 스트림 열기
2. **broadcast**: 스트림을 순회하며 각 비디오를 YouTube Live로 순서대로 송출
...
...세 비디오가 YouTube Live 채널로 이어서 송출됩니다. 이전 비디오가 방송 중일 때 추가로 `publish-video`를 호출하면 큐에 쌓였다가 방송기가 현재 비디오를 마치는 즉시 이어서 송출되므로, 끊김 없는 24/7 스타일의 방송을 만들 수 있습니다.

## 커스터마이징

- 백프레셔 여유 공간을 조정하려면 `video-queue.max_size`를 늘리거나 줄이세요
