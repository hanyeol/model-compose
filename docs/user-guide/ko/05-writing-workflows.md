...
이 장에서는 model-compose의 워크플로우 작성 방법을 다룹니다. 단일 작업부터 복잡한 다중 단계 워크플로우까지, 작업 간 데이터 전달, 조건부 실행, 스트리밍 모드, 에러 핸들링을 학습합니다.
...
이 세 가지 요소를 조합하여 간단한 단일 작업부터 복잡한 다중 단계 파이프라인까지 다양한 워크플로우를 구성할 수 있습니다.
