...
- `output` projection (per-item template rendering)
- Direct pass-through when `output` is absent or equals `${output[]}`
...
class TestFilterOutputProjection:
    ...
    @pytest.mark.anyio
    async def test_output_equals_output_bracket_is_direct(self):
        """`output: ${output[]}` should behave identically to omitting output."""
        workflow = {
            "id": "wf",
            "jobs": [
                {
    ...
    @pytest.mark.anyio
    async def test_output_projects_item_fields_via_output_bracket(self):
        """Without a `where` clause, `${item}` is not registered — projection
        must reference the surviving item via `${output[]}` instead."""
        workflow = {
            "id": "wf",
            "jobs": [
                {
    ...
    @pytest.mark.anyio
    async def test_output_can_reference_output_bracket_inside_composite(self):
        """`${output[]}` inside a composite output refers to the surviving item."""
        workflow = {
            "id": "wf",
            "jobs": [
                {
        ...
        items = await _collect(result)
        assert items == [
            { "raw": { "id": 1, "meta": { "flag": True } }, "id": 1 },
            { "raw": { "id": 2, "meta": { "flag": True } }, "id": 2 },
        ]

    @pytest.mark.anyio
    async def test_output_projection_combined_with_where(self):
        workflow = {
            "id": "wf",
            "jobs": [
                {
...
class TestFilterStreaming:
    ...
    @pytest.mark.anyio
    async def test_streaming_with_output_projection(self):
        workflow = {
            "id": "wf",
            "jobs": [
                {
        ...
        items = await _collect(result)
        assert items == [
            { "id": 2, "raw": { "id": 2, "score": 20 } },
            { "id": 3, "raw": { "id": 3, "score": 30 } },
        ]
...
class TestFilterUpstream:
    @pytest.mark.anyio
    async def test_consumes_upstream_json_list(self):
        components = [
            {
                "id": "emit",
                "type": "shell",
                "action": {
                    "command": [
                        "echo",
                        '[{"id":1,"keep":true},{"id":2,"keep":false},{"id":3,"keep":true}]',
                    ],
