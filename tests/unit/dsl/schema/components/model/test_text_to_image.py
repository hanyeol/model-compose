"""Unit tests for the Huggingface text-to-image component/action schemas.
...
COMPONENT_ADAPTER = TypeAdapter(TextToImageModelComponentConfig)
...
class TestArchitectureDiscriminator:
    def test_sdxl_component_resolves_to_sdxl_action(self):
        ...
        assert isinstance(cfg, SdxlHuggingfaceTextToImageModelComponentConfig)
        assert len(cfg.actions) == 1
        assert isinstance(cfg.actions[0], SdxlHuggingfaceTextToImageModelActionConfig)
    ...
    def test_flux_component_resolves_to_flux_action(self):
        ...
        assert isinstance(cfg, FluxHuggingfaceTextToImageModelComponentConfig)
        assert isinstance(cfg.actions[0], FluxHuggingfaceTextToImageModelActionConfig)
    ...
    def test_hunyuan_image_component_resolves_to_hunyuan_action(self):
        ...
        assert isinstance(cfg, HunyuanImageHuggingfaceTextToImageModelComponentConfig)
        assert isinstance(cfg.actions[0], HunyuanImageHuggingfaceTextToImageModelActionConfig)
...
class TestTaskDiscriminator:
    def test_legacy_task_image_generation_rejected(self):
        with pytest.raises(ValidationError):
            COMPONENT_ADAPTER.validate_python(_base("sdxl", {
...
class TestArchitectureEnum:
    def test_enum_values(self):
        assert HuggingfaceTextToImageModelArchitecture.SDXL == "sdxl"
        assert HuggingfaceTextToImageModelArchitecture.FLUX == "flux"
        assert HuggingfaceTextToImageModelArchitecture.HUNYUAN_IMAGE == "hunyuan-image"
