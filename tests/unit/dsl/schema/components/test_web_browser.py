...
class TestWebBrowserClickActionConfig:
    ...
    def test_invalid_no_target(self):
        ...
        with pytest.raises(ValidationError, match="One of 'selector', 'xpath', or coordinates"):
    ...
    def test_invalid_multiple_targets(self):
        ...
        with pytest.raises(ValidationError, match="Only one of 'selector', 'xpath', or coordinates"):
            WebBrowserClickActionConfig(
                method="click",
                selector="button",
                xpath="//button"
...
class TestWebBrowserScrollActionConfig:
    ...
    def test_defaults(self):
        """Test scroll action defaults: None for all targeting fields, validator requires at least one."""
        config = WebBrowserScrollActionConfig(method="scroll", x=0, y=0)
        assert config.x == 0
        assert config.y == 0
        assert config.selector is None
        assert config.xpath is None

        with pytest.raises(ValidationError, match="At least one of 'selector', 'xpath', or coordinates"):
            WebBrowserScrollActionConfig(method="scroll")
...
class TestWebBrowserActionConfigDiscriminator:
    ...
    def test_discriminator_scroll(self):
        ...
        config = ActionAdapter.validate_python({"method": "scroll", "x": 0, "y": 0})
