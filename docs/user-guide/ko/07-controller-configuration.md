...
| `name` | string | `controller-queue` | 작업 큐의 기본 이름. 큐 키: `{name}:{workflow_id}`. 결과 키: `{name}:{workflow_id}:{run_id}` |
| `result_ttl` | string | `1h` | 결과 항목 TTL (예: `"1h"`, `"30m"`). `"0s"` = 만료 없음 |
| `max_concurrent_count` | integer | `1` | 최대 동시 처리 작업 수 (>= 1) |
| `worker_id` | string | 자동 | 워커 고유 식별자 (자동 ULID 생성) |
| `workflows` | list | `null` | 워커가 처리할 워크플로우 ID. 미지정 시 모든 비공개(private) 아닌 워크플로우 사용. 단일 ID는 `workflow:` 축약형 사용. |
...
| `pop_timeout` | string/number | `1s` | BRPOP 타임아웃 (예: `"1s"`, `"500ms"`) |
...
|------|------|--------|------|
| `driver` | string | **필수** | 큐 백엔드 드라이버 (`redis`) |
| `name` | string | `controller-queue` | 작업 큐 기본 이름. 큐 키: `{name}:{workflow_id}`. 결과 키: `{name}:{workflow_id}:{run_id}` |
| `timeout` | string/number | `0s` | 결과 대기 최대 시간 (예: `"30s"`). `"0s"` = 제한 없음 |
| `max_blob_size` | string/number | `50M` | 큐에 저장할 개별 블롭 최대 크기 (예: `"512K"`, `"2G"`) |
