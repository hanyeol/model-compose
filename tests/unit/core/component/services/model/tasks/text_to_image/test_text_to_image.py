"""Tests for the TextToImageTaskAction's I/O matrix.
...
from mindor.core.component.services.model.tasks.text_to_image.common import TextToImageTaskAction
from mindor.dsl.schema.action import SdxlHuggingfaceTextToImageModelActionConfig
...
class _FakeTextToImageAction(TextToImageTaskAction):
    ...
    def __init__(self, config: SdxlHuggingfaceTextToImageModelActionConfig):
...
def _make_config(
    ...
    *,
    ...
    if output is not None:
        raw["output"] = output
    return SdxlHuggingfaceTextToImageModelActionConfig.model_validate(raw)
...
class TestSingleInput:
    ...
    async def test_no_output_returns_single_image(self):
        action = _FakeTextToImageAction(_make_config("${input.prompt}"))
    ...
    async def test_literal_prompt_returns_single_image(self):
        action = _FakeTextToImageAction(_make_config("a cat"))
...
class TestListInput:
    ...
    async def test_returns_list_of_images(self):
        action = _FakeTextToImageAction(_make_config("${input.prompts}"))
    ...
    async def test_uneven_last_batch(self):
        action = _FakeTextToImageAction(_make_config("${input.prompts}", batch_size=3))
...
class TestStreamInput:
    ...
    async def test_direct_output_returns_async_iterator(self):
        action = _FakeTextToImageAction(_make_config("${input.prompts}", output="${result}"))
...
class TestOutputExpression:
    ...
    async def test_result_reference_returns_direct(self):
        action = _FakeTextToImageAction(_make_config("hello", output="${result}"))
