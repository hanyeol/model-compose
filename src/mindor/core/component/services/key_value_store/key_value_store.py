...
from .base import KeyValueStoreDriver, KeyValueStoreDriverRegistry
import importlib
...
class KeyValueStoreComponent(ComponentService):
    def __init__(
        ...
        self.driver: KeyValueStoreDriver = self._create_driver(self.config.driver)

    def _create_driver(self, driver: KeyValueStoreDriverType) -> KeyValueStoreDriver:
        try:
            if driver not in KeyValueStoreDriverRegistry:
                self._load_driver_module(driver)
            return KeyValueStoreDriverRegistry[driver](self.id, self.config, self.daemon)
        except KeyError:
            raise ValueError(f"Unsupported key-value store driver: {driver}")

    def _load_driver_module(self, driver: KeyValueStoreDriverType) -> None:
        """Import the module that registers the given key-value store driver.

        Convention: a driver "foo-bar" (KeyValueStoreDriverType.value) maps to
        ...
        populating KeyValueStoreDriverRegistry.
        """
