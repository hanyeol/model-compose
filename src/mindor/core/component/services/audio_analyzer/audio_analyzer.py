...
from .base import AudioAnalyzerDriver, AudioAnalyzerDriverRegistry
import importlib
...
class AudioAnalyzerComponent(ComponentService):
    def __init__(
        ...
        self.driver: AudioAnalyzerDriver = self._create_driver(self.config.driver)

    def _create_driver(self, driver: AudioAnalyzerDriverType) -> AudioAnalyzerDriver:
        try:
            if driver not in AudioAnalyzerDriverRegistry:
                self._load_driver_module(driver)
            return AudioAnalyzerDriverRegistry[driver](self.id, self.config, self.daemon)
        except KeyError:
            raise ValueError(f"Unsupported audio analyzer driver: {driver}")

    def _load_driver_module(self, driver: AudioAnalyzerDriverType) -> None:
        """Import the module that registers the given audio analyzer driver.

        Convention: a driver "foo-bar" (AudioAnalyzerDriverType.value) maps to
        ...
        Importing the module triggers its @register_audio_analyzer_driver
        decorator, populating AudioAnalyzerDriverRegistry.
        """
