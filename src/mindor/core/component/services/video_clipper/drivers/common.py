...
class VideoClipperAction(ComponentAction):
    def __init__(self, config: VideoClipperActionConfig):
        self.config: VideoClipperActionConfig = config
    ...
    async def run(self, context: ComponentActionContext) -> Any:
        ...
        # so it can produce the right per-video shape directly.
    ...
    async def _format_result(result: Any, merge: bool, is_single_span: bool, return_timestamp: bool) -> Any:
        ...
        what the caller actually asked for:
          ...
          - is_single_span: result is a one-element async iterator of clips;
            pull the one out.
          - otherwise: result is an async iterator of `{video, start_time,
            end_time}` clips.
        When `return_timestamp` is off, strip the span metadata down to just
        the video.
        """
        if merge:
            return result if return_timestamp else result["video"]
    ...
    async def _clip_batch(
        ...
        `{video, start_time, end_time}`; a merged clip carries all covered
        spans as `{video, times: [{start_time, end_time}, ...]}`. Callers
