...
class TestSingleInput:
    ...
    # NOTE: ``output="${result[]}"`` on single/list (collect-mode) input cannot be
    # exercised here because that re-rendering path requires a real VariableRenderer
    # to pluck array elements from the registered ``result`` source — our fake mock
    # cannot synthesize that AsyncIterator. The streaming-input path below covers
...
class TestListInput:
    ...
    # NOTE: see TestSingleInput — ``output="${result[]}"`` on collect-mode input
    # requires a real VariableRenderer to convert the registered ``result`` list into
    # an AsyncIterator. The TestStreamInput.test_stream_output_template_returns_*
    # below covers the streaming-input equivalent.
...
class TestTokenStreaming:
    ...
    @pytest.mark.anyio
    async def test_stream_input_streaming_yields_per_row_chunk_iterators(self):
        """AsyncIterator input + streaming: each row is a StreamChunkIterator over its tokens."""
        def _make_iter():
            return _make_async_iter([ "a", "bb" ])
    ...
    # NOTE: ``${result[]}`` on collect-mode + streaming requires the real
    # VariableRenderer to convert the registered ``result`` list of StreamChunkIterators
    # into a flattening AsyncIterator. The mock cannot synthesize that — the streaming-
    # input variant above (``test_stream_input_streaming_yields_per_row_chunk_iterators``)
    # covers the closest path that the unit fake can express end-to-end.
