...
class RedisQueueSubscriberControllerAdapterConfig(CommonQueueSubscriberControllerAdapterConfig):
    ...
    host: str = Field(default="localhost", description="Redis server hostname or IP.")
    port: int = Field(default=6379, ge=1, le=65535, description="Redis server port.")
    secure: bool = Field(default=False, description="Use TLS/SSL (equivalent to rediss:// protocol).")
    database: int = Field(default=0, ge=0, le=15, description="Redis database number.")
    password: Optional[str] = Field(default=None, description="Redis password. Can also be set in the URL.")
    pop_timeout: Union[str, int, float] = Field(default="1s", description="BRPOP timeout before retrying (e.g. '1s', '500ms').")
