...
class SdxlImageGenerationTaskService(ImageGenerationTaskService):
    ...
    def _load_pretrained_pipeline(self) -> Tuple[StableDiffusionXLPipeline, torch.device]:
        from diffusers import StableDiffusionXLPipeline
        import torch
        ...
        logging.info(f"Component '{self.id}': loading SDXL pipeline from {source}")

        pipeline = StableDiffusionXLPipeline.from_pretrained(source, **params)
    ...
    def _resolve_pipeline_source(self) -> str:
        ...
        raise ValueError(f"Unsupported model config type for SDXL: {type(self.config.model).__name__}")
    ...
    async def _run(
        ...
        return await SdxlImageGenerationTaskAction(action, self.pipeline, self.device).run(context, loop)
