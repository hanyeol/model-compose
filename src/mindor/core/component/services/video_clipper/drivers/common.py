...
class VideoClipperAction(ComponentAction):
    ...
    async def run(self, context: ComponentActionContext) -> Any:
        ...
        # A single span dict collapses the per-video result into a scalar clip;
    ...
    @staticmethod
    async def _collapse(clips: AsyncIterator[VideoStreamResource]) -> VideoStreamResource:
        """Pull the single clip out of a one-element iterator produced by _clip_batch."""
        first: Optional[VideoStreamResource] = None
    ...
    async def _clip_batch(
        ...
        """Return, per input video, an async iterator of clips."""
        pass
