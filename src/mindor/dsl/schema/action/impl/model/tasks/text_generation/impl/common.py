...
class TextGenerationParamsConfig(BaseModel):
    ...
    temperature: Union[float, str] = Field(default=0.7, description="Sampling temperature; higher values produce more random outputs.")
...
class TextGenerationModelActionConfig(CommonModelActionConfig):
    prompt: Union[str, List[str]] = Field(..., description="Input prompt or prompts the model generates text from.")
    ...
    batch_size: Union[int, str] = Field(default=1, description="Number of input prompts processed per batch.")
