...
The audio analyzer component measures signal-level properties of an audio file — loudness, peak level, gain, and silence — and returns a compact summary. Use it for mastering QA, level normalization decisions, silence trimming, and any pipeline that needs to *inspect* audio without transforming it.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `metric` | string | **required** | Measurement to run: `loudness`, `peak`, `gain`, `silence` |
...
RMS, peak, headroom, DC offset, flat factor. No configurable fields beyond the common ones.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `threshold` | float | `-30.0` | Amplitude threshold in dBFS below which audio is considered silent |
...
```python
{
  ...
  "silent_ratio":      0.068,
  "regions": [
    {"start_time": 0.0,   "end_time": 3.2,   "duration": 3.2},
    {"start_time": 88.7,  "end_time": 91.5,  "duration": 2.8},
    ...
  ],
}
```
