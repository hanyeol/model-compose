...
YAML 파일 하나. 어떤 모델이든, 어떤 프로토콜이든, 어떤 런타임이든.

챗 API, RAG 파이프라인, 자율 에이전트, MCP 서버를 오케스트레이션 코드 없이 만들고 — `docker-compose`처럼 같은 정의를 어디에서나 실행하세요.
...
- **Portable** — AI 시스템을 한 번 정의하고, 재설계 없이 어디에서나 실행합니다.
- **Model-Agnostic** — 로컬 모델과 클라우드 API를 하나의 시스템에서 함께 사용합니다.
- **Stream-Native** — 토큰, 오디오, 비디오 프레임, 이벤트가 워크플로우를 따라 1급 값으로 흐릅니다.
...
- **100+ 컴포넌트** — 모델, 에이전트, HTTP/WebSocket 클라이언트, 벡터/그래프 스토어, 셸, 브라우저 등
