...
| `image-drawing` | `${result}` | - | 도형·텍스트가 그려진 이미지 |
...
- 로컬 실행 컴포넌트 (`model`, `datasets`, `text-splitter`, `image-processor`, `image-drawing`, `image-compressor`) → `${result}`
...
조건식은 하나 이상의 조건을 평가하여 처음으로 매칭되는 분기를 방출합니다. 딕셔너리 안에서 `"?"` 키를 사용하며, 값은 조건 객체 하나 또는 여러 개의 리스트입니다 (리스트는 위에서 아래로 평가되며, 처음 매칭되는 조건이 이깁니다).
...
role:
  "?":
    input: ${item.role}
    value: tool
    if_true: user               # 매칭 → "user"
    if_false: ${item.role}      # 그 외 → 원래 role 전달
```
...
| 필드 | 필수 | 기본값 | 설명 |
|------|:----:|--------|------|
| `input` | 예 | — | 평가 대상 값. 대개 `${...}` 표현식. |
| `operator` | 아니요 | `eq` | `eq`, `neq`, `gt`, `gte`, `lt`, `lte`, `in`, `not-in`, `starts-with`, `ends-with`, `match` 중 하나. |
| `value` | 대개 | — | 비교의 우측 값. 값이 필요 없는 연산자에는 생략 가능. |
| `if_true` | 아니요 | — | 조건이 매칭됐을 때 방출할 값. |
| `if_false` | 아니요 | — | 조건이 매칭되지 않았을 때 방출할 값. 생략하고 매칭도 실패하면 전체 `"?"` 는 아무 것도 기여하지 않습니다 (아래 설명 참고). |

### 14.10.2 다중 조건 (첫 매칭이 이김)
...
각 블록 타입은 매칭되는 첫 번째 분기에 의해 다시 쓰여집니다. 세 타입 어디에도 매칭되지 않는 블록은 상위 딕셔너리에 아무 것도 기여하지 않습니다.

### 14.10.3 값 위치 vs 형제 위치

`"?"`의 동작은 상위 딕셔너리에 다른 키가 있는지에 따라 달라집니다.

- **형제 위치** — `"?"`가 다른 키들과 함께 있음. 선택된 분기는 **반드시 딕셔너리로 해석되어야** 하며, 그 필드들은 상위 딕셔너리에 병합됩니다 (`"..."` 스프레드처럼).

  ```yaml
  "...": ${item}                # item의 모든 필드로 시작
  "?":                          # 이후 선택적으로 오버라이드
    input: ${item.role}
    value: tool
    if_true: { role: user }
...
- **값 위치** — `"?"`가 상위 딕셔너리의 유일한 키. 선택된 분기가 상위 필드의 값을 **완전히 대체**합니다. 값 위치에서 조건식이 필요하면 딕셔너리로 감쌉니다.
...
### 14.10.4 매칭되는 분기가 없을 때

어떤 조건도 매칭되지 않고 `if_false`도 주어지지 않으면 `"?"`는 아무 것도 방출하지 않습니다:

- **형제 위치** → 상위 딕셔너리에 아무 필드도 추가되지 않음.
- **값 위치** → 상위 필드가 상위 딕셔너리에서 아예 생략됨 (`null`이 방출되지 않음).

이 특성 덕분에 `"?"`로 빈 필드를 추가하지 않고 선택적 장식을 얹을 수 있습니다.

### 14.10.5 참고

- `"?"`는 **맵 아이템 필터가 아닙니다** — 반복되는 각 요소에 항상 (병합, 대체 또는 생략의 방식으로) 기여합니다. 리스트에서 아이템을 걸러내려면 `"*"`의 `where` 절로 필터링하세요.
- 리스트로 여러 조건을 넣으면 처음 매칭되는 조건에서 평가가 중단됩니다.

---

## 14.11 조인 연산자 (`"+"`)

`"+"` 키는 동일 타입의 여러 조각을 하나의 값으로 이어 붙입니다. 조인 종류는 요소 타입으로 결정됩니다: 문자열은 이어붙임, 리스트는 확장, 딕셔너리는 병합(뒤가 이김). 여러 `"*"` 맵 결과, 선택적 조각, 리터럴 조각들을 별도 변환 단계 없이 한번에 엮을 때 유용합니다.
...
가장 흔한 사용 사례: 같은 소스에 여러 `"*"` 맵을 돌린 뒤 결과를 하나의 리스트로 이어붙이기. `"?"` 체인으로 분기 처리하던 매핑을 훨씬 깔끔하게 표현합니다.

```yaml
# 표준 assistant 블록 → OpenAI assistant 메시지 필드.
content:
  "+":
    - "*":
        input: ${item.content}
...
- `"+"`의 값은 리스트로 해석되어야 합니다 — YAML 리터럴 리스트든, 리스트로 해석되는 `${...}` 표현식이든 상관없습니다. 그 외 값은 `TypeError`를 발생시킵니다.
- **`None` 요소는 건너뜁니다.** 이 덕분에 선택적 조각(`${maybe_missing}`, `{ "?": {...} }`)을 별도 처리 없이 그대로 넣을 수 있습니다.
- **결과가 비면** (요소가 없거나 모두 `None`으로 해석된 경우) 전체 `"+"`는 `None`으로 렌더링됩니다. 대체값이 필요하면 상위 호출부에서 `| default`로 이어붙이세요.
- **동일 타입만 허용** — 살아남은 요소는 모두 같은 종류여야 합니다 (`str`, `list`, `dict` 중 하나). 섞이면 `TypeError`. 종류별로 조인 의미가 다르기 때문에 혼합은 모호합니다.
  - `str` 요소 → 구분자 없이 이어붙임 (필요하면 리터럴로 직접 구분자를 넣으세요).
  - `list` 요소 → 확장 (`[1, 2] + [3] → [1, 2, 3]`).
  - `dict` 요소 → 뒤가 이기는 병합, `"..."`과 동일한 의미.
- `"+"`는 **키 전용**이며 반드시 딕셔너리의 유일한 키여야 합니다. `{ "+": [...], "other": 1 }`은 조인이 아니라 일반 딕셔너리 리터럴로 취급됩니다.

---

## 14.12 Zip 연산자 (`"&"`)

`"&"` 키는 **정렬된 여러 소스의 dict**를 받아, 위치별로 하나씩 dict를 만든 리스트(또는 스트림)를 생성합니다 — 각 출력 dict는 호출자가 지정한 키에 각 소스의 한 요소를 담습니다. 이미 정렬된 여러 시퀀스가 있고 이들을 함께 순회하고 싶을 때, 템플릿에 인덱스를 흘려보내거나 `"*"` 맵으로 각 소스를 위치로 재조회하지 않고 바로 쓸 수 있습니다.
...
rows:
  "&":
    num: ${a}
    letter: ${b}
# 결과: [{num: 1, letter: "x"}, {num: 2, letter: "y"}, {num: 3, letter: "z"}]
```

출력 키는 `"&"` dict에서 오고, 값은 소스 시퀀스를 가리킵니다. 소스는 3개, 4개 이상 얼마든지 가능합니다.

### 14.12.2 정렬된 시퀀스 짝짓기

전형적인 형태: 같은 원본에 대해 별도의 잡이 만들어낸 프레임별 메타데이터와 프레임별 검출 결과를 짝지어 묶습니다.
...
요소는 그대로 전달됩니다 — `"&"`는 값 자체를 재구성하지 않고 위치 짝만 만듭니다.

### 14.12.3 스트림 Zip

소스 중 하나 이상이 스트림이면 `"&"`는 위치별로 dict 하나씩 방출하는 스트림을 반환하고, 어느 한 소스라도 소진되는 순간 종료합니다. 스트림과 이미 구체화된 리스트를 자유롭게 섞을 수 있습니다.

```yaml
# a: 토큰 스트림, b: 토큰별 log-prob 스트림
tokens:
  "&":
    token: ${a}
    logprob: ${b}
# 결과: <{token, logprob} 스트림>
```

스트림은 출력 행마다 라운드로빈으로 전진하므로 특정 소스만 앞서 버퍼링되지 않습니다.

### 14.12.4 `"*"` 및 `"+"`와의 조합

`"&"`는 `"*"` (맵)과 자연스럽게 결합해 짝지어진 각 행을 재구성하고, `"+"` (조인)와 함께 짝지어진 행들을 다운스트림 리스트나 문자열로 접을 수 있습니다.

```yaml
# 짝짓고 나서 각 행을 어시스턴트 메시지 파트로 재구성
parts:
  "*":
    "&":
      text: ${chunks}
...
- **소스는 비어 있지 않은 dict로 해석되어야 합니다.** 리스트 인자(`{ "&": [...] }`)나 스칼라는 `TypeError` — dict 키가 곧 출력 필드 이름이기 때문입니다.
- **빈 dict는 `ValueError`.** `{ "&": {} }`는 출력 키가 없어 의미가 없습니다.
- **`None`은 단락합니다.**
  - `"&"`의 전체 값이 `None`으로 해석되면(예: `${missing}`) 결과는 `None`입니다.
  - dict *내부의 어떤 소스*라도 `None`으로 해석되면 결과는 `[]` — 한 레인이 비면 생성할 짝지어진 행이 없습니다.
- **가장 짧은 소스가 승리합니다.** 어느 한 소스가 소진되면 zip이 멈추고, 더 긴 소스의 남은 요소는 버려집니다. Python의 `zip` 및 스트리밍 시맨틱과 동일합니다.
- **요소 identity는 보존됩니다.** 값은 출력 키 아래로 그대로 배치되며, 중첩된 dict/list/스트림은 풀리지 않습니다.
- **Fragmentation은 승계됩니다.** 소스 중 하나라도 fragmented 스트림(`is_fragmented=True`)이면 zip 출력도 fragmented로 표시되어 다운스트림 소비자가 조각으로 도착함을 알 수 있습니다.
- `"&"`는 **키 전용**이며 자신이 속한 dict의 유일한 키여야 합니다. `{ "&": {...}, "other": 1 }`은 zip이 아니라 일반 dict 리터럴로 취급됩니다.
