...
class TextRerankingModelActionConfig(CommonModelActionConfig):
    ...
    documents: Union[List[str], List[Dict[str, Any]], str] = Field(..., description="Candidate documents to rerank. List of strings or objects.")
    document_field: Optional[str] = Field(default=None, description="When documents are objects, the field name containing the text to score.")
    ...
    max_input_length: Optional[Union[int, str]] = Field(default=None, description="Maximum tokens per (query, document) pair. None uses the tokenizer's model_max_length.")
