...
class SpeechSeparationTaskAction:
    def __init__(self, config: SpeechSeparationModelActionConfig, device: Optional[torch.device]):
        self.config: SpeechSeparationModelActionConfig = config
        ...
        if isinstance(audio, (StreamIterator, AsyncIterator)):
            async def _stream_output_generator():
                async for batch_audios in BatchSourceIterator(audio, batch_size=batch_size or 1):
                    batch_results = await self._separate(batch_audios, params, streaming, loop)
    ...
    async def _resolve_params(self, context: ComponentActionContext) -> Dict[str, Any]:
        ...
        num_speakers = await context.render_variable(self.config.params.num_speakers)
    ...
    @abstractmethod
    async def _separate(self, audios: List[MediaSource], params: Dict[str, Any], streaming: bool, loop: asyncio.AbstractEventLoop) -> Union[List[List[Dict[str, Any]]], List[Union[Iterator[Dict[str, Any]], AsyncIterator[Dict[str, Any]]]]]:
        pass

class SpeechSeparationTaskService(ModelTaskService):
    pass
