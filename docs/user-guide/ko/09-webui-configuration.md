...
워크플로우에서 출력을 `as stream/text` 또는 `as stream/json`로 지정하면 실시간 스트리밍으로 표시됩니다.

```yaml
workflow:
  title: Summarize Text
  input: ${input}
  output: ${output as stream/text}  # 스트리밍 출력
...
- **텍스트 스트리밍** (`stream/text`): 청크가 도착할 때마다 누적하여 표시 (예: AI 텍스트 생성)
- **JSON 스트리밍** (`stream/json`): 각 청크를 리스트에 추가하여 표시 (예: 여러 결과 순차 생성)
