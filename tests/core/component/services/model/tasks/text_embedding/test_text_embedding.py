...
class TestSingleInput:
    ...
    async def test_stream_output_yields_one(self):
        action = _FakeEmbeddingAction(_make_config("${input.text}", output="${result[]}"))
        ctx    = ComponentActionContext("r-3", { "text": "world" })
        loop   = asyncio.get_running_loop()
        result = await action.run(ctx, loop)

        assert isinstance(result, AsyncIterator)
        items = await _collect(result)
        assert items == [ [ 5.0, 0.0 ] ]
...
class TestListInput:
    ...
    @pytest.mark.anyio
    async def test_stream_output_yields_each_embedding(self):
        action = _FakeEmbeddingAction(_make_config("${input.texts}", output="${result[]}"))
        ctx    = ComponentActionContext("r-6", { "texts": [ "a", "bb", "ccc" ] })
        loop   = asyncio.get_running_loop()
        result = await action.run(ctx, loop)
        ...
        items = await _collect(result)
        assert len(items) == 3
        assert items == [ [ 1.0, 0.0 ], [ 2.0, 1.0 ], [ 3.0, 0.0 ] ]
