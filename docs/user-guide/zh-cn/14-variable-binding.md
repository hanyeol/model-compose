...
| `image-drawing` | `${result}` | - | 绘制了图形/文本的图像 |
...
- 本地执行组件 (`model`, `datasets`, `text-splitter`, `image-processor`, `image-drawing`, `image-compressor`) → `${result}`
...
`"&"` 键接收一个**并行源的 dict**，并按位置生成一个由 dict 组成的列表（或流）——每个输出 dict 在调用者指定的键下承载每个源的一个元素。当你已经有几个对齐的序列想要一起遍历时使用；无需把索引穿进模板，也无需用 `"*"` 映射按位置去重新查找每个源。
...
rows:
  "&":
    num: ${a}
    letter: ${b}
# 结果: [{num: 1, letter: "x"}, {num: 2, letter: "y"}, {num: 3, letter: "z"}]
```

输出键来自 `"&"` dict，值命名源序列。源可以任意多个——三个、四个甚至更多都可以。

### 14.12.2 对齐序列的配对

典型形状：把由两个独立作业针对同一原始素材产出的每帧元数据和每帧检测结果配对。
...
元素原样传递——`"&"` 从不重构值本身，只做位置上的配对。

### 14.12.3 流的 Zip

当一个或多个源是流时，`"&"` 返回一个流，按对齐位置产出一个 dict，并在任一源耗尽时立即终止。源可以自由混合流和已物化的列表。

```yaml
# a: token 的流，b: 每个 token 的 log-prob 流
tokens:
  "&":
    token: ${a}
    logprob: ${b}
...
流按输出行轮询推进，因此不会有某个源被提前缓冲。

### 14.12.4 与 `"*"` 及 `"+"` 组合

`"&"` 天然搭配 `"*"`（map）来重塑每个对齐行，也可与 `"+"`（join）一起把 zip 出的行折叠成下游列表或字符串。

```yaml
# 先 zip，再把每一行重塑为助手消息片段
parts:
  "*":
    "&":
      text: ${chunks}
...
- **源必须解析为非空 dict。** 列表参数（`{ "&": [...] }`）或标量会抛出 `TypeError`——因为 dict 的键就是输出字段的名称。
- **空 dict 会抛出 `ValueError`。** `{ "&": {} }` 没有输出键，毫无意义。
- **`None` 会短路。**
  - 若整个 `"&"` 值解析为 `None`（如 `${missing}`），结果为 `None`。
  - 若 dict *内部的任一源*解析为 `None`，结果为 `[]`——某个通道缺失就意味着没有可产出的对齐行。
- **最短的源获胜。** 任一源耗尽，zip 就停止；更长源中的多余元素被丢弃。与 Python 的 `zip` 及流式语义一致。
- **元素身份被保留。** 值原样置于输出键之下；嵌套的 dict/list/流不会被解包。
- **分片属性会被继承。** 若任一源为分片流（`is_fragmented=True`），zip 输出也会被标记为分片，以便下游消费者知晓数据是分块到达的。
- `"&"` 是**仅键**，必须是其所在 dict 的唯一键。`{ "&": {...}, "other": 1 }` 会被当作普通 dict 字面量而非 zip。
