...
component:
  ...
  # Optional IP-Adapter — supply an action-level `reference_image` to steer appearance.
  ip_adapter:
    provider: huggingface
...
For AniGen, pick the SS-Flow and SLAT-Flow variants on the component and toggle which outputs the action returns:
...
component:
  ...
  ss_variant: solo      # solo (default, accurate geometry), epic, duet
  slat_variant: auto    # auto (default, network picks joint count), control
  action:
    image: ${input.image as image}
    seed: ${input.seed as integer}
    return_mesh: true
...
**Supported families and presets:**
- `pixal3d` — Pixal3D single-image to textured GLB. Requires a CUDA GPU with ~18 GB VRAM at 1536 resolution (default), or ~10-12 GB with `low_vram: true` at 1024 resolution.
...
| `bs-roformer` | Configurable stem separation | lucidrains' Band-Split RoFormer. Architecture-only package — point `model` at a pretrained `.ckpt`/`.safetensors` and match `params` to the checkpoint |
| `mel-band-roformer` | Configurable stem separation | Mel-band variant of BS-RoFormer. Requires `params.sample_rate` to match the checkpoint (mel filter bank is baked in at construction time) |
...
| Family | Backend | Notes |
|--------|---------|-------|
| `beat-this` | CPJKU Beat This! (ISMIR 2024) | Transformer-based joint beat + downbeat estimator; checkpoints auto-download from HuggingFace; optional madmom DBN post-processing via `dbn: true` |

The action returns a dict per input with a `beats` list — each event carries `time` (seconds), `is_downbeat` (`true` at each measure start), and `beat_number` (position within the measure, `1` on downbeats and `2, 3, ...` on subsequent beats; `null` for pickup beats before the first downbeat). When `return_metadata: true`, `duration` is also included.
