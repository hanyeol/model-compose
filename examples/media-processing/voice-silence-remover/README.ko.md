...
이 예제는 **Silero VAD**와 **`audio-clipper`** 컴포넌트를 연결하여, 음성 녹음에서 무음 구간을 제거하는 워크플로우를 보여줍니다. 무음, 숨소리, 배경 소음은 제거되고 남은 음성 클립들은 하나의 오디오로 병합됩니다.

## 개요

워크플로우는 두 개의 작업으로 구성됩니다:

1. **`detect-silence`** — Silero VAD 모델을 로컬에서 실행해, 입력 오디오에서 `{start_time, end_time, confidence}` 형태의 음성 구간 리스트를 생성합니다 (그 사이의 비음성 구간이 제거 대상 무음입니다).
2. **`refine`** — 그 구간 리스트를 그대로 `audio-clipper`의 `span`으로 넘기고 `merge: true`를 지정해, ffmpeg가 모든 음성 클립을 하나의 무음 제거 오디오로 이어붙이도록 합니다.
...
2. **워크플로우 실행:**
   ...
   **웹 UI 사용:**
   ...
   - **Run Workflow** 클릭 후 무음이 제거된 오디오 다운로드
...
### "Voice Silence Remover" 워크플로우

**설명**: Silero VAD로 음성 구간을 감지하고, 이를 병합해 무음이 제거된 하나의 오디오 파일을 생성합니다.
...
### 무음이 제거된 오디오를 다운스트림 ASR에 연결

세 번째 작업을 추가해 `${jobs.refine.output.audio}`를 `speech-to-text` 모델 컴포넌트의 입력으로 넘기면 됩니다. 무음이 제거된 오디오에 ASR을 돌리면 비용과 hallucination이 모두 감소하는 경향이 있습니다.
...
- **Whisper 전처리**: 무음이 제거된 오디오를 Whisper에 넣을 계획이라면 `min_silence_duration`을 약간 크게(`1s` 정도) 잡아 발화 내부 짧은 쉼을 여러 개의 미세 클립으로 쪼개지 않도록 하세요.
