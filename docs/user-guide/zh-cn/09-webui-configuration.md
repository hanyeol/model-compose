...
当工作流输出指定为 `as stream/text` 或 `as stream/json` 时，会实时流式显示。

```yaml
workflow:
  title: Summarize Text
  input: ${input}
  output: ${output as stream/text}  # 流式输出
...
- **文本流式传输** (`stream/text`)：累积并显示到达的块（例如，AI 文本生成）
- **JSON 流式传输** (`stream/json`)：将每个块添加到列表中并显示（例如，多个结果的顺序生成）
