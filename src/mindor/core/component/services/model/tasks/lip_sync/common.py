...
from mindor.dsl.schema.action import LipSyncModelActionConfig
    ...
    async def run(self, context: ComponentActionContext) -> Any:
        ...
        if isinstance(video, (StreamIterator, AsyncIterator)):
            async def _stream_output_generator():
                async for batch_videos, batch_audios in BatchSourceIterator(source, batch_size=batch_size or 1):
                    batch_results = await self._generate_batch(batch_videos, batch_audios, params, context.cancellation_token)
