...
YoloObjectTrackingTaskAction. Segment fields must live at the top of the chunk
...
class TestSegmentChunkIsFlat:
    def test_segment_chunk_has_no_nested_segment_key(self):
        ...
        chunk = action._build_segment_chunk(3, _tracked_segment(), _params())

        assert "segment" not in chunk

    def test_segment_chunk_carries_expected_flat_fields(self):
        action = YoloObjectTrackingTaskAction.__new__(YoloObjectTrackingTaskAction)
        chunk = action._build_segment_chunk(3, _tracked_segment(score=0.77), _params())

        assert chunk["type"] == "segment"
        assert chunk["track_id"] == 3
        ...
        assert chunk["label"] == "car"
        assert chunk["label_id"] == 2
