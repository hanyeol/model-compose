...
```bash
# Prepare document chunks for vector embedding
model-compose run --input '{
  "text": "Large knowledge base document...",
  "chunk_size": 400,
  "chunk_overlap": 50
}'
```
...
```bash
# Split document for hierarchical summarization
model-compose run --input '{
...
```bash
# Split large document for translation
model-compose run --input '{
  ...
  - id: document-embedding
    jobs:
      ...
      - id: generate-embeddings
        ...
        input:
          text: ${jobs.split-text.output.chunks[*]}
...
### Quality Assurance
- **Boundary Validation**: Check for clean text breaks
