...
from mindor.core.utils.media import MediaSource, create_media_source
from mindor.core.utils.streaming import BytesStreamResource, FileStreamResource
...
async def _drain_resource_to_file(resource: AudioStreamResource) -> str:
    """Copy a resource's underlying file to a stable temp path so we can probe it."""
    src = resource.source
...
class TestFFmpegAudioExtractor:
    ...
    async def test_invalid_input_raises_runtime_error(self, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="ffmpeg audio extraction failed"):
            await FFmpegAudioExtractorAction(config).run(ctx)
...
class TestStreamOutput:
    ...
    @pytest.mark.anyio
    async def test_stream_output_returns_async_iterator(self, sample_video_path):
        config = _make_config(output="${result[]}", format="mp3")
        ctx = _make_context([sample_video_path, sample_video_path])
    ...
    @pytest.mark.anyio
    async def test_stream_output_with_single_input_yields_one(self, sample_video_path):
        config = _make_config(output="${result[]}", format="mp3")
        ctx = _make_context(sample_video_path)
