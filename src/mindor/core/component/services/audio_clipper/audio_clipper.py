...
from .base import AudioClipperService, AudioClipperServiceRegistry
import importlib
...
class AudioClipperComponent(ComponentService):
    def __init__(
        ...
        self.service: AudioClipperService = self._create_service(self.config.driver)

    def _create_service(self, driver: AudioClipperDriver) -> AudioClipperService:
        try:
            if driver not in AudioClipperServiceRegistry:
                self._load_driver_module(driver)
            return AudioClipperServiceRegistry[driver](self.id, self.config, self.daemon)
        except KeyError:
            raise ValueError(f"Unsupported audio clipper driver: {driver}")

    def _load_driver_module(self, driver: AudioClipperDriver) -> None:
        """Import the module that registers the given audio clipper driver.

        Convention: a driver "foo-bar" (AudioClipperDriver.value) maps to
        mindor.core.component.services.audio_clipper.drivers.foo_bar — either
        a single-file module (foo_bar.py) or a package (foo_bar/__init__.py).
        Importing the module triggers its @register_audio_clipper_service
        decorator, populating AudioClipperServiceRegistry.
        """
