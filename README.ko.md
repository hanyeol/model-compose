...
AI 워크플로우, 에이전트, 모델, 도구를 하나의 YAML 파일로 정의하고, 스택을 다시 작성하지 않고도 로컬, 컨테이너, 프로덕션 어디서든 실행할 수 있습니다.

`docker-compose`에서 영감을 받은 선언적 접근 방식을 채택하여, 하나의 YAML 파일이 전체 AI 시스템을 정의합니다.
