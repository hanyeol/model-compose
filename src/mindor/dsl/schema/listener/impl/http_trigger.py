...
class HttpTriggerConfig(BaseModel):
    ...
    bulk: Union[bool, str] = Field(default=False, description="Whether this trigger handles multiple items per request.")
    item: Optional[str] = Field(default=None, description="Field path to extract individual items from the payload.")
    workflow: str = Field(..., description="Workflow ID to execute when triggered.")
...
class HttpTriggerListenerConfig(CommonListenerConfig):
    ...
    port: int = Field(default=8091, ge=1, le=65535, description="Port on which the HTTP server listens.")
    base_path: Optional[str] = Field(default=None, description="Base path prefix for all trigger endpoints.")
    triggers: List[HttpTriggerConfig] = Field(default_factory=list, description="Trigger endpoint configurations.")
